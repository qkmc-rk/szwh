...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<div class="Index_top_1">
			...
			<div class="header" style="background:url('<%=request.getContextPath() %>/static/front/images/yszz.jpg') no-repeat center top;">
					...
					<div class="y_indexi_2" style="min-height:520px; _height:520px; padding-bottom:10px;">
						...
						<div id="pagger" style="text-align: center">
							<div style="width:100%;float:left;">
								...
								<a style="margin-right:5px;" href="<%=request.getContextPath() %>/article_movie?currentPage=1">首页</a>
								
								<c:if test="${page.currentPage == 1  }">
									<a style="margin-right:5px;" href="<%=request.getContextPath() %>/article_movie?currentPage=1">上一页</a>
								</c:if>
								<c:if test="${page.currentPage > 1  }">
									<a style="margin-right:5px;" href="<%=request.getContextPath() %>/article_movie?currentPage=${page.currentPage - 1}">上一页</a>
								</c:if>
								...
								<c:forEach varStatus="status" begin="1" end="${page.totalPage }">
									...
									<c:if test="${page.currentPage != status.index && status.index <= 5}">
										<a href="<%=request.getContextPath() %>/article_movie?currentPage=${status.index}" style="margin-right:5px;">${status.index}</a>
									</c:if>
								...
								<c:if test="${page.currentPage == page.totalPage  }">
									<a style="margin-right:5px;" href="<%=request.getContextPath() %>/article_movie?currentPage=${page.totalPage}">下一页</a>
								</c:if>
								<c:if test="${page.currentPage < page.totalPage  }">
									<a style="margin-right:5px;" href="<%=request.getContextPath() %>/article_movie?currentPage=${page.currentPage + 1}">下一页</a>
								</c:if>
								
								<a style="margin-right:5px;" href="<%=request.getContextPath() %>/article_movie?currentPage=${page.totalPage}">尾页</a>&nbsp;&nbsp;转到
